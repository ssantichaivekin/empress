...
##############################################################################################
# end of tanglegram layout algorithms/heuristics.
# other stuff for drawing.
def drawS(newickS, dfsS, drawScale = 1.0, drawScale2 = 1.0):

    #dfsS.clear()
    # wcc: hardcode hecking
    dfsS.edgeRadius = .5 * drawScale
    dfsS.nodeRadius = 1. * drawScale
    dfsS.stepX = 20      #* drawScale / 20.                        # flip the direction, S is on the right
    dfsS.stepY = -20       #* drawScale / 20.
    dfsS.originY = (dfsS.stepY - dfsS.stepY * len(newickS.leaves)) / 2
    dfsS.originX = +5 * len(newickS.leaves) / 10.    * drawScale2                        # wcc: hardcode

    # draw S
    newickS.dfs_traverse(dfsS)

    
def drawG(newickG, dfsG, drawScale = 1.0, drawScale2 = 1.0):

    #dfsG.clear()
    # wcc: hardcode
    dfsG.edgeRadius = .5 * drawScale
    dfsG.nodeRadius = 1. * drawScale
    dfsG.stepX = -20       #* drawScale / 20.                     # changing the direction, G is on the left
    dfsG.stepY = -20       #* drawScale / 20.
    dfsG.originY = (dfsS.stepY - dfsG.stepY * len(newickG.leaves)) / 2
    dfsG.originX = -5 * len(newickG.leaves) / 10.     * drawScale2                     # wcc: hardcode
    
    # draw G
    newickG.dfs_traverse(dfsG)
    
def drawT(newickG, dfsT, drawScale = 1.0):
    
    dfsT.clear()
    # wcc: hardcode
    dfsT.tangleRadiusL = .5 * drawScale
    dfsT.tangleRadiusN = .5 * drawScale

    # draw the connections
    newickG.dfs_traverse(dfsT)

def redraw(newickS, newickG, drawScale, drawScale2):
    drawS(newickS, dfsS, drawScale, drawScale2)
    drawG(newickG, dfsG, drawScale, drawScale2)
    drawT(newickG, dfsT, drawScale)


def list_to_ranks(L):
    """
    convert the list into the dictionary that preserves the given order
    NOTICE: ranks start at 1 to n
    """
    ranks = {}
    for x in L:
        assert (not ranks.has_key(x)), "Error: duplicated item, %s" % x
        ranks[x] = len(ranks) + 1   # wcc: notice the +1
    return ranks
   
if __name__=='__main__':

    import vpython4.vtanglegram as v
    ...
    #T2.insert(2)
    #T2.insert(5)
    #print crossAVL(T1, T2)
    #
    ##sys.exit()

    # need to consider different input styles now due to multi-multi
    leafMap = None
    if len(sys.argv) == 2:
        f = open(sys.argv[1], 'r')
        txtS = f.next()
        ...
        for line in f:
            rtn = line.split(':')
            if len(rtn) == 2:
                leafMap.append((rtn[0].strip().strip('"').strip("'"), \
                                rtn[1].strip().strip('"').strip("'")))
        f.close()

    elif len(sys.argv) == 3:
        txtS = open(sys.argv[1], 'r').next()
        txtG = open(sys.argv[2], 'r').next()

    else:
        print "Usage: tanglegram.py <tree file> [2nd tree file] \n"
        raw_input("Press any key to continue with default example... ")
        txtS = open(INPUT_S, 'r').next()
        txtG = open(INPUT_G, 'r').next()

    treeS, treeG = newick_utils.newick_leaf_map(txtS, txtG, leafMap)
    print "Read species tree:\n", newick_utils.plot_ascii_tree(treeS, attr="interleaf")
    print "Read gene trees:\n",   newick_utils.plot_ascii_tree(treeG, attr="interleaf")
    print "Read leaf mapping:", leafMap

    #sys.exit()
    
    #print "Testing ..."
    #print "taxon size:", len(ranks)
    #print "leaf set size in tree 0:", len(newickG.leaves)

    #for x in leafRank:
    #    print x, leafRank[x]
    #print leafRank.keys()
    #print newickGs[0].leaves
    #for x in newickG.leaves_ids:
    #    if not ranks.has_key(x.strip()):
    #        print x,
    #    else:
    #        print ranks[x],
    #print

    print "Starting tanglegram processing ..."
    # untangle the tree?

    #build_leaf_rank(newickS, newickG)
    #dfsTanglegram = DFS_tanglegram(optimize = "MIN_MOD", solver = "LIST")
    #dfsResetter = DFS_reset()

    curr_mode = 1
    t1 = time.time()
    crossing = tanglegram1T(treeS, treeG, optimize="MIN_MOD", solver="LIST")[0]
    #dfsTanglegram(newickG)
    t2 = time.time()

    print "total crossing: %d, time: %f, solver: %s" % ( crossing, t2 - t1, "LIST" )
    print "Tree S:\n", newick_utils.plot_ascii_tree(treeS, attr="interleaf")
    print "Tree G:\n", newick_utils.plot_ascii_tree(treeG, attr="interleaf")
    ...
    #dfsTanglegram.reset(optimize = "MIN_MOD", solver = "LIST")
    
    #t3 = time.time()
    #dfsTanglegram(newickG)
    #t4 = time.time()

    #print "total crossing: %d, time: %f , solver: %s" % ( dfsTanglegram.total_cross, t4 - t3, dfsTanglegram.solver )

    #sys.exit()

    print "LCA mapping ..."
    v.build_LCA(treeG, treeS, verbose=True) # from vtanglegram.py

    # only works when verbose=True in build_LCA
    for n in treeG.nodes:
        print n, "->", n.M

    #sys.exit()

    print "Calling vpython ..."

    v.scene.toolbar = True
    v.scene.autoscale = True
    v.scene.background = v.color.white

    # the basic axes
    xaxis = v.arrow(pos = (0.05, 0, 0), axis = (1, 0, 0), color = v.color.red)
    yaxis = v.arrow(pos = (0, 0.05, 0), axis = (0, 1, 0), color = v.color.green)
    zaxis = v.arrow(pos = (0, 0, 0.05), axis = (0, 0, 1), color = v.color.blue)
    ...
    drawLabel = True
    drawScale = max(len(treeS.leaves), len(treeG.leaves)) / 5.
    drawScale2 = 5.0

    dfsS = v.DFS_visual(frame = frameS, style = 'ultrametric')

    # render the same tree again, but mirrored
    dfsG = v.DFS_visual(frame = frameG, style = 'ultrametric')

    dfsT = v.DFS_tanglegram_visual(frame = frameT, lca=True)

    dfsG.nodeColor = dfsS.nodeColor = v.color.green
    #dfsG.labelColor = dfsS.labelColor = v.color.black
    dfsT.tangleColorL = v.color.red


    drawS(treeS, dfsS, drawScale)
    drawG(treeG, dfsG, drawScale)

    drawT(treeG, dfsT, drawScale)
    
    print "forward:", v.scene.forward
    print "up:",      v.scene.up
    # change scene.forward first allows changing scene.up
...
#                if subObj.__class__ == label:
#                    labels.append(subObj)
    ...
    while True:
        ...
        if v.scene.kb.keys:
            ...
            if s == 't' or s == 'T':
                print "top view"
                v.scene.up = (0, 0, 1)
                v.scene.forward = (0, 0.0001, -1)
                
            if s == 'l' or s == 'L':
                drawLabel = not drawLabel
                print "draw labels:", drawLabel
                ...
                #frameL.visible = not frameL.visible
                for l in treeS.leaves:
                    l.vlbl.visible = drawLabel
                for l in treeG.leaves:
                    l.vlbl.visible = drawLabel

            if s == ',' or s == '<':
                # rotate the camera -z
                print "rotating CW in z"
                v.scene.forward = v.scene.forward.rotate(axis = (0, 0, -1), angle = pi/8)
            ...
            if s == 'n' or s == 'N':
                dfsT.lca = not dfsT.lca
                redraw(treeS, treeG, drawScale, drawScale2)

            if s == 'q' or s == 'Q':
                print "status:"
                print "scene.up:",      repr(v.scene.up)
                print "scene.forward:", repr(v.scene.forward)
                print "scene.center:",  repr(v.scene.center)
                print "G taxa:",        len(treeG.leaves)
                print "S taxa:",        len(treeS.leaves)
                print "optimize:",      dfsTanglegram.optimize
                print "total crossings:", dfsTanglegram.total_cross

            if s == '\t':
                MODES = ["NONE", "MAX_MOD", "MIN_MOD"]
                curr_mode = (curr_mode + 1) % 3
                new_optimize = MODES[curr_mode]
                print "switching optimization to:", new_optimize
                #dfsTanglegram.reset(optimize = new_optimize, verbose = False, solver = "LIST")
                newick_utils.random_internal_switch(treeS)
                newick_utils.random_internal_switch(treeG)
                crossing = tanglegram2TE(treeS, treeG, optimize=new_optimize, solver="LIST")[0]
                crossing = tanglegram1T (treeS, treeG, optimize="NONE", solver="LIST")[0]
                #print "optimize:", dfsTanglegram.optimize

                #dfsResetter(newickG)
                #dfsTanglegram(newickG)

                print "total crossings:", crossing
                # redraw
                drawG(treeG, dfsG, drawScale)
                drawT(treeG, dfsT, drawScale)
                redraw(treeS, treeG, drawScale, drawScale2)
            ...
            if s in ['up', 'down', 'left', 'right']:
                xaxis.pos = yaxis.pos = zaxis.pos = v.scene.center
