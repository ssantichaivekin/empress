...
def add_reconcile_to_parser(reconcile_parser: argparse.ArgumentParser):
    ...
    reconcile_parser.add_argument("--outfile", metavar="<filename>",
...
def run_reconcile(args):
    ...
    print("Output to {}".format(args.outfile))
    ...
    if args.graph:
        recon_graph.export_csv(args.outfile)
    else:
        median = recon_graph.median()
        median.export_csv(args.outfile)
