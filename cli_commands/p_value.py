...
def add_p_value_to_parser(p_value_parser: argparse.ArgumentParser):
    ...
    p_value_parser.add_argument("--nsamples", metavar="<number of samples>", type=int,
                                help="Number of random mappings to sample. Defaults to 100", default=100)
...
def run_p_value(args):
    ...
    recongraph.draw_stats_on(ax, args.nsamples)
    plt.savefig(outfile)
    plt.close()
