...
class Tree(object):
    ...
    def __repr__(self):
        ...
        for (n,b,l) in self.edges:
            ...
            sep = ', '
        tree_str +=')'
        # wcc: display the internal node id
        if self.id:
            tree_str += "'" + self.id + "'"
        return tree_str
...
class _TreeBuilder(parser.AbstractHandler):
    ...
    def new_tree_end(self, name):
        # wcc: to add internal label
        if name and name != '' and name != '_':
            self.stack[-1].id = name

    def new_edge(self,bootstrap,length):
        n = self.stack.pop()
        # wcc: to handle the case (...)root:length
        if len(self.stack) > 0:
...
# wcc: add base_dist for default branch-length
def add_distance_from_root(tree, base_dist = 1.0):
    '''Extend all nodes with the distance (branch length) from the root'''
    tree.distance_from_root = 0.0       # 'tree' is the root...
    class V(TreeVisitor):
        def pre_visit_edge(self,src,b,l,dst):
            # wcc: default brench length set to 1
            if l is None: l = base_dist
