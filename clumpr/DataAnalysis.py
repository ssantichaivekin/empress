...
def read_file(csv_file, mpr_strip=0, mpr_equals_median_strip=False):
    ...
    ignore_mprs = []
    if mpr_strip > 0:
        ignore_mprs = list(map(str, list(range(0, mpr_strip))))
    ...
    name_to_row = {v: k for k, v in column.items()}
...
def analyse_data(csv_file, given_properties, non_normalized, timings, plot, latex, strip_mprs, strip_equal):
    ...
    mpr_list = [float(e) for e in mpr_list]
    gene_count_list = properties_that_exist_in_file["Gene Node Count"]
    gene_count_list= [float(e) for e in gene_count_list]
    ...
    DP_timings = [float(e) for e in DP_timings]
    prop_list_dict = {}
    timing_list_dict = {}
    ...
    for property in given_properties:
        ...
        if "Diameter" in property or "Count" in property or "Number" in property or "Distance" in property:
            prop_list_dict[property] = [float(e) for e in prop_list_dict[property]]
            if timings:
    ...
    if timings:
        for timing_list in timing_list_dict:
            timing_list_dict[timing_list] = [float(e) for e in timing_list_dict[timing_list]]
    ...
    if plot:
        ...
        for prop in plot_info_list:
            ...
            y_limits = prop[3]
            prop_data = prop[4]
            prop_normalized = [prop_data[i[0]] / prop_normalized_against[i[0]] for i in enumerate(prop_data)]
            if timings:
