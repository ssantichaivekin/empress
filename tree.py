...
class Node:
    ...
    # Takes in a dictionary with keys as node names and values of either 0 or 1 to swap each node for minimizing crossings
    def swap_fix(self, swap_dict):
        if not self.is_leaf:
            if swap_dict[self.name] == 0:
                self.swap()
            if self.left_node is not None:
                self.left_node.swap_fix(swap_dict)
            if self.right_node is not None:
                self.right_node.swap_fix(swap_dict)
